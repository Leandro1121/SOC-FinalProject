...
static void mix_columns(uint8_t s[16]) {
	for (int c = 0; c < 4; c++) {
		...
		uint8_t a2 = s[4*c+2];
		uint8_t a3 = s[4*c+3];
		s[4*c+0] = gf_mul(a0,2) ^ gf_mul(a1,3) ^ a2 ^ a3;
...
static void inv_mix_columns(uint8_t s[16]) {
	for (int c = 0; c < 4; c++) {
		...
		uint8_t a2 = s[4*c+2];
		uint8_t a3 = s[4*c+3];
		s[4*c+0] = gf_mul(a0,14) ^ gf_mul(a1,11) ^ gf_mul(a2,13) ^ gf_mul(a3,9);
...
static void key_expansion_128(const uint8_t *key, uint8_t round_keys[176]) {
...
void aes128_encrypt_sw(const uint8_t *key, const uint8_t *in, uint8_t *out) {
	uint8_t state[16];
	uint8_t round_keys[176];

	for (int i = 0; i < 16; i++) state[i] = in[i];
	key_expansion_128(key, round_keys);
...
void aes128_decrypt_sw(const uint8_t *key, const uint8_t *in, uint8_t *out) {
	uint8_t state[16];
	uint8_t round_keys[176];
