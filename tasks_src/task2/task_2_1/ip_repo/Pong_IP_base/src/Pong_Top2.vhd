...
entity Pong_Top2 is
  ...
  port (
    ...
    --seven-seg output
    o_sev_seg_P1: out std_logic_vector(6 downto 0);
    o_sev_seg_P2: out std_logic_vector(6 downto 0);
...
architecture rtl of Pong_Top2 is
  ...
  signal  o_score_P1	:  std_logic_vector(3 downto 0);
  signal  o_score_P2	:  std_logic_vector(3 downto 0);
...
--  end process p_Reg_Syncs; 
 ...
 --#Add_H Instantiation of seven-seg module
    sev_seg_P1_int	: sev_seg_mod
    port map (
    digit_value => o_score_P1,
    sev_seg	=> o_sev_seg_P1
    );

    sev_seg_P2_int	: sev_seg_mod
    port map (
    digit_value => o_score_P2,
    sev_seg	=> o_sev_seg_P2
    );
...
--#H#  w_Draw_Any <= w_Draw_Ball or w_Draw_Paddle_P1 or w_Draw_Paddle_P2;
   
  -- Assign Color outputs, only two colors, White or Black
--#H#  o_Red_Video <= (others => '1') when w_Draw_Any = '1' else (others => '0');
--#H#  o_Blu_Video <= (others => '1') when w_Draw_Any = '1' else (others => '0');
--#H#  o_Grn_Video <= (others => '1') when w_Draw_Any = '1' else (others => '0');
 
-- #AddedByH# Addition for Grad section 
 -- Paddles: yellow 
 -- Ball: Green
 -- Background: Black
 
 o_Red_Video <= (others => '1') when (w_Draw_Paddle_P1 = '1' or w_Draw_Paddle_P2 = '1') else
(others => '0') when w_Draw_Ball = '1' else
(others => '0');

o_Grn_Video <= (others => '1') when (w_Draw_Paddle_P1 = '1' or w_Draw_Paddle_P2 = '1' or w_Draw_Ball = '1') else
(others => '0');

o_Blu_Video <= (others => '0');  


 --#AddedByH# Display P1,P2 scores in 7-segs
  D0_an <= "1110" ;  
  D1_an <= "1110" ;  
  o_score_P1 <= std_logic_vector(to_unsigned(r_P1_Score mod 10,4));
  o_score_P2 <= std_logic_vector(to_unsigned(r_P2_Score mod 10,4));
